...
import { Modal, View, Pressable, Text, StyleSheet, Image, Linking, Platform, ActivityIndicator, useWindowDimensions } from 'react-native';
...
export function BusinessCardModal() {
  ...
  // Hooks
  const { business: enrichedBusiness, loading: detailsLoading, fetchDetails, hasDetails } = useBusinessDetails(businessForHook || {} as BusinessProps, false);
  ...
  const backContent = (
    <View style={styles.cardContent}>
      ...
      <View style={styles.backRating}>
        <StarRating rating={business.rating || 0} />
        <Text style={styles.backReviewText}>{business.review_count} Review{business.review_count > 1 ? 's' : ''}</Text>
      </View>
